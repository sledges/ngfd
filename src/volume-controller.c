...
retry_timeout_cb (gpointer userdata)
{
    ...
    if (!connect_to_pulseaudio (context)) {
        LOG_WARNING ("%s >> failed to reconnect, trying again in %d seconds", __FUNCTION__, RETRY_TIMEOUT);
        context->volume_retry_id = g_timeout_add_seconds (RETRY_TIMEOUT, retry_timeout_cb, context);
    }
    else {
        LOG_DEBUG ("%s >> reconnected to pulseaudio.", __FUNCTION__);
...
filter_cb (DBusConnection *connection, DBusMessage *msg, void *data)
{
    ...
    {
        LOG_DEBUG ("%s >> pulseaudio disconnected, reconnecting in %d seconds", __FUNCTION__, RETRY_TIMEOUT);
...
add_entry (Context *context, const char *role, guint volume)
{
    ...
    if (!reply) {
        if (dbus_error_is_set (&error)) {
            LOG_WARNING ("%s >> error: %s", __FUNCTION__, error.message);
        }

        goto done;
    }

    LOG_DEBUG ("%s >> volume for role %s set to %d", __FUNCTION__, role, vol);
    success = TRUE;
...
process_queued_ops (Context *context)
{
    ...
    while ((queued_volume = g_queue_pop_head (context->volume_queue)) != NULL) {
        LOG_DEBUG ("%s >> processing queued volume (role=%s, volume=%d)", __FUNCTION__, queued_volume->role, queued_volume->level);
...
connect_to_pulseaudio (Context *context)
{
    ...
    if (dbus_error_is_set (&error)) {
        LOG_WARNING ("%s >> failed to open connection to pulseaudio: %s", __FUNCTION__, error.message);
        dbus_error_free (&error);
        return FALSE;
    }
    ...
    if (!dbus_connection_add_filter (context->volume_bus, filter_cb, context, NULL)) {
        LOG_WARNING ("%s >> failed to add filter", __FUNCTION__);
        return FALSE;
    }
...
volume_controller_create (Context *context)
{
    ...
    if (!connect_to_pulseaudio (context)) {
        LOG_DEBUG ("%s >> failed to connect to Pulseaudio DBus, reconnecting in %d seconds.", __FUNCTION__, RETRY_TIMEOUT);
        context->volume_retry_id = g_timeout_add_seconds (RETRY_TIMEOUT, retry_timeout_cb, context);
    }
...
volume_controller_update (Context *context, Volume *volume)
{
    ...
    if (!context->volume_bus) {
        LOG_DEBUG ("%s >> volume controller not ready, queueing op.", __FUNCTION__);
