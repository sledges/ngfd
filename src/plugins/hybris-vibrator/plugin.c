...
#define AV_KEY "plugin.hybris-vibrator.data"
#define LOG_CAT  "hybris-vibrator: "

typedef struct _HybrisVibratorData
{
    NRequest       *request;
...
N_PLUGIN_NAME        ("hybris-vibrator")
N_PLUGIN_VERSION     ("0.1")
N_PLUGIN_DESCRIPTION ("Haptic feedback using Droid Vibrator HAL via libhybris")

static int
hybris_vibrator_sink_initialize (NSinkInterface *iface)
{
    (void) iface;
...
static void
hybris_vibrator_sink_shutdown (NSinkInterface *iface)
{
    (void) iface;
...
static int
hybris_vibrator_sink_can_handle (NSinkInterface *iface, NRequest *request)
{
    (void) iface;
    (void) request;
...
static int
hybris_vibrator_sink_prepare (NSinkInterface *iface, NRequest *request)
{
    N_DEBUG (LOG_CAT "sink prepare");

    HybrisVibratorData *data = g_slice_new0 (HybrisVibratorData);
...
timeout_cb (gpointer userdata)
{
    ...
    HybrisVibratorData *data = (HybrisVibratorData*) userdata;
    g_assert (data != NULL);
...
static int
hybris_vibrator_sink_play (NSinkInterface *iface, NRequest *request)
{
    ...
    HybrisVibratorData *data = (HybrisVibratorData*) n_request_get_data (request, AV_KEY);
    g_assert (data != NULL);
...
static int
hybris_vibrator_sink_pause (NSinkInterface *iface, NRequest *request)
{
...
static void
hybris_vibrator_sink_stop (NSinkInterface *iface, NRequest *request)
{
    ...
    g_slice_free (HybrisVibratorData, data);
}
...
N_PLUGIN_LOAD (plugin)
{
    ...
    static const NSinkInterfaceDecl decl = {
        ...
        .initialize = hybris_vibrator_sink_initialize,
        .shutdown   = hybris_vibrator_sink_shutdown,
        .can_handle = hybris_vibrator_sink_can_handle,
        .prepare    = hybris_vibrator_sink_prepare,
        .play       = hybris_vibrator_sink_play,
        .pause      = hybris_vibrator_sink_pause,
        .stop       = hybris_vibrator_sink_stop
    };
