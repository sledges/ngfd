...
static void
_add_property_int (NgfEventPrototype *prototype,
                   GKeyFile          *k,
                   const char        *group,
                   const char        *key,
                   gint               def_value,
                   gboolean           set_default)
{
    ...
    value = ngf_value_new ();
    ngf_value_set_int (value, result);

    g_hash_table_insert (prototype->properties, g_strdup (key), value);
}

static void
_add_property_bool (NgfEventPrototype *prototype,
                    GKeyFile          *k,
                    const char        *group,
                    const char        *key,
                    gboolean           def_value,
                    gboolean           set_default)
{
    ...
    value = ngf_value_new ();
    ngf_value_set_boolean (value, result);

    g_hash_table_insert (prototype->properties, g_strdup (key), value);
}

static void
_add_property_string (NgfEventPrototype *prototype,
                      GKeyFile          *k,
                      const char        *group,
                      const char        *key,
                      ...
                      gboolean           set_default)
{
    ...
    value = ngf_value_new ();
    ngf_value_set_string (value, result);
 ...
 * list out of them.
 *
 * @param prototype NgfEventPrototype
 * @param k GKeyFile
 * @param group Group name
 * @param prefix Prefix to match the properties
 * @param default_role Default audio stream role
 * @post New pa_proplist set to the NgfEventPrototype.
 */

static void
_parse_stream_properties (NgfEventPrototype *prototype,
                          GKeyFile          *k,
                          const char        *group,
                          const char        *prefix,
...
_parse_single_prototype (SettingsData *data, GKeyFile *k, GList **prototypes_done, GHashTable *prototypes, const char *name)
{
    ...
    _parse_stream_properties (p, k, group, "stream.", ngf_properties_get_string (p->properties, "audio_stream_role"));

    /* If we have a parent prototype, make a copy of it and merge our new prototype
       to the copy. */

    if (parent != NULL) {
        copy = ngf_event_prototype_copy (ngf_daemon_get_prototype (self, parent));

        if (copy != NULL) {
            ngf_event_prototype_merge (copy, p);
            ngf_event_prototype_free (p);
            p = copy;
        }
    }
